...
# Query PBP
pbp_query <- dbSendQuery(conn, "SELECT * FROM pbp WHERE season == '20162017'")
...
save(list = c("player_df",
     ...
     file = "~/Documents/corsica_data/war_data.RData"
     )
...
# load("~/Documents/corsica_data/war_data.RData")
...
# %Dev = 0.0229200
away_rate_coefs <- matrix(exp(coef(cox_away_rates, s = "lambda.min")))
...
contracts <- read.csv("~/Documents/capfriendly_contracts.csv")

league_min <- data.frame(season = c("20122013",
                                    "20132014",
                                    "20142015",
                                    "20152016",
                                    "20162017"
...
contracts %>%
  mutate(sign_year = as.character(lubridate::year(as.Date(Signing.Date, "%y-%m-%d"))),
         sign_season = paste0(sign_year, nabs(sign_year) + 1, sep = "")
         ) %>%
  filter({sign_season == "20112012" & Cap.Hit == 525000 & Length == 2} |
         {sign_season == "20122013" & Cap.Hit == 525000} |
         {sign_season == "20132014" & Cap.Hit == 550000}
         ) %>%
  data.frame() ->
  min_contracts

bind_rows(min_contracts %>%
            filter(Length %in% 1:2) %>%
            mutate(season = sign_season) %>%
            data.frame(),
          min_contracts %>%
            filter(Length == 2) %>%
            mutate(season = as.character(nabs(sign_season) + 10001)) %>%
            data.frame()
          ) %>%
  data.frame() %>%
  filter(season %in% league_min$season) %>%
  mutate(player = paste(toupper(First.Name), toupper(Last.Name), sep = ".")) %>%
  data.frame() ->
  contracts_sum

contracts_sum %>%
  select(season, player, POS, Cap.Hit) %>%
  arrange(season) %>%
  data.frame()
...
# load(url("http://fenwicka.com/shiny/war_coefs.RData"))
...
merge(rates_sum %>%
  ...
  mutate(OWAR = WAR_RF + WAR_QF + WAR_Shooting + WAR_PD + WAR_OZF,
