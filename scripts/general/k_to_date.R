...
foreach(i = sort(unique(as.Date(na.omit(newpbp$game_date)))), .combine = "rbind") %do% {
  
  cat(as.character(i), "\n")
  cat("Data Prep: ")
  
  range <- as.Date(min(as.Date(na.omit(newpbp$game_date))):as.Date(i), origin = "1970-01-01")
  
  pbp <- filter(newpbp, as.Date(game_date) %in% range)
...
## Save
save(list = "k_df",
     file = paste("~/corsica_data/k_to_date_",
                  season,
                  ".RData",
                  sep = ""
                  )
